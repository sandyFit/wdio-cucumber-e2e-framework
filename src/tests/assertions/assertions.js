...
export function assertElementCount(elements, expectedCount, elementName = 'elements') {
    expect(elements.length).to.equal(expectedCount, `Expected ${expectedCount} ${elementName}, but found ${elements.length}`);
}

export function assertMinimumElements(elements, minCount, elementName = 'elements') {
    assert.isAtLeast(elements.length, minCount, `Expected at least ${minCount} ${elementName}, but found ${elements.length}`);
}
