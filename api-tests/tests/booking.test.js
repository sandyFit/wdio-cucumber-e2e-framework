...
describe(']Restful-Booker API Tests', () => {
  ...
  describe('Update Booking', () => {
    it('should fully update booking with PUT using Basic Auth', async () => {
      ...
      const response = await bookingService.updateBooking(createdBookingId, updatedData, authToken);
    ...
    it('should return 405 when deleting non-existent booking', async () => {
      try {
        ...
        if (error.response && error.response.duration) {
          expect(error.response.duration).to.be.below(config.expectedResponseTime.fast);
        }
