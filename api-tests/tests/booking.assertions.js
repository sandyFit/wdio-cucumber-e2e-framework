...
const assertResponse = {
  ...
  responseTime: (response, threshold = "medium") => {
    expect(response.duration).to.be.below(
      config.expectedResponseTime[threshold],
    );
  },
  ...
  bookingDataMatches: (responseData, expectedData) => {
    ...
    if (expectedData.bookingdates) {
      if (expectedData.bookingdates.checkin) {
        expect(responseData.bookingdates.checkin).to.equal(
          expectedData.bookingdates.checkin,
        );
      }
      if (expectedData.bookingdates.checkout) {
        expect(responseData.bookingdates.checkout).to.equal(
